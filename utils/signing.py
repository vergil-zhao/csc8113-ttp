...
class Key:
    ...
    def __init__(self, public_key: str = None, private_key: str = None):
        ...
        if public_key is not None:
            self.__public_key = VerifyingKey.from_string(
                self.decompress(b64decode(public_key))[1:],
                curve=NIST256p
            )
    ...
    def public_key(self) -> str:
        ...
        # return b64encode(b'\x04' + self.__public_key.to_string()).decode('ascii')
        return b64encode(self.compress(b'\x04' + self.__public_key.to_string())).decode('ascii')

    @property
    def private_key(self) -> Optional[str]:
        if self.__private_key is not None:
            return b64encode(self.__private_key.to_string()).decode('ascii')
        return None
